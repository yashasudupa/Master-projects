...
from tools import split_unescape
...
parser.add_option("-r", "--refbase", dest="refbase", action="store_true",
                  help="Enable KAI Refbase tests")
...
if Path(documentsPath).is_dir():
    import fnmatch
    (_, _, allPDFs) = next(os.walk(documentsPath))
    allPDFs = fnmatch.filter(allPDFs, "*.pdf")
else:
    allPDFs = []
    ...
    if line.startswith("@"):
        ...
        if currentId in usedIds or not usedIds:
            if currentType in requiredFields:
                ...
                if refbase:
                    if refbase and "refbase" not in requiredFields[currentType]:
                        requiredFields[currentType].append("refbase")
        ...
        if currentId in usedIds or not usedIds:
            if "=" in line:
                ...
                if field == "openaccess":
                    if not value.startswith("done:"):
                        # parse and compare date
                        oadate = dateutil.parser.parse(value)
                        if oadate < datetime.now():
                            subproblems.append("Open Access: document must be published online ("+value+")")
                ...
                if field == "file":
                    files = value.split(';')
                    for f in files:
                        vals = split_unescape(f, ':')
                        if len(vals) > 3:
                            subproblems.append("more than 3 fields: " + str(vals))
                            counterDocumentLinkErrors += 1
                        try:
                            name, file, type = vals[0], vals[1], vals[2]
                        except IndexError as e:
                            print("Error: "+currentId+str(vals))
                            raise e
                        file = file.replace("\\\\", "/")
                        if type.lower() == "pdf":
                            if documentsPath not in file:
                                subproblems.append("file is not in "+documentsPath+": " + file)
                                counterDocumentLinkErrors += 1
                            else:
                                fs  = file.split(documentsPath)
                                if len(fs) > 1:
                                    file = fs[1].strip("\\/")
                                    if file in allPDFs:
                                        allPDFs.remove(file)
                                    pdf = Path('.') / documentsPath / file
                                    if not pdf.is_file():
                                        subproblems.append("file not found: " + file)
                                        counterDocumentLinkErrors += 1
                if refbase:
                    if field == "refbase":
                        try:
                            ...
                            subproblems.append("failed to parse refbase '{}'".format(value))
                            counterFlawedNames += 1
...
for file in allPDFs:
    counterDocumentLinkErrors += 1
    if toconsole:
        print("PROBLEM: file not linked in bibtex: " + file)


problemCount = counterMissingFields + counterFlawedNames + counterWrongTypes + counterNonUniqueId + counterDocumentLinkErrors
...
""")
    ...
    html.write("<li># document link errors: "+str(counterDocumentLinkErrors)+"</li>")
