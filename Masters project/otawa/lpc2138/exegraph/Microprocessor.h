/*
 *	$Id: Microprocessor.h,v 1.6 2007/06/27 09:35:25 casse Exp $
 ...
 */
#ifndef OTAWA_LPC2138_MICROPROCESSOR_H
#define OTAWA_LPC2138_MICROPROCESSOR_H
...
inline instruction_category_t instCategory(Inst *inst) {
...
class Microprocessor {
 ...
 public:
  Microprocessor(void);
  Microprocessor(const otawa::hard::Processor *proc);
...
template <class N>
Microprocessor<N>::Microprocessor(const otawa::hard::Processor *proc)
:	_pipeline_stage_index(0),
	_operand_reading_stage(0),
	_operand_producing_stage(0)
{
	...
	for(int i = 0; i < ostages.count(); i++) {
		...
		if(is_exec) {
			...
			// Build the FU
			const Table<otawa::hard::FunctionalUnit *>& fus = ostages[i]->getFUs();
			...
			// Build the bindings
			const Table<otawa::hard::Dispatch *>& dispatch = ostages[i]->getDispatch();
...
#endif // OTAWA_LPC2138_MICROPROCESSOR_H
