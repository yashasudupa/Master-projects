 ...
 */
#ifndef OTAWA_LPC2138__EXEGRAPH_H__
#define OTAWA_LPC2138__EXEGRAPH_H__
...
class ExeGraph  {
 public:
  ...
  public: 
    inline ExeEdge(N *source, N *target, edge_type_t type)
      : _latency(0), GenGraph<N, ExeEdge>::GenEdge(source,target), _type(type) {
  ...
  public:
    inline ExeNode(ExeGraph<N> *graph, PipelineStage<N> *stage, ExeInst<N> *inst)
      ...
      _produces_operands(false), GenGraph<N,ExeEdge>::GenNode((otawa::graph::Graph *)graph->graph()) {
 ...
 public:
  ...
  class Predecessor: public GenGraph<N,ExeEdge>::InIterator {
  public:
    inline Predecessor(const N* node)
      : GenGraph<N,ExeEdge>::InIterator(node) {}
  };
  class Successor: public GenGraph<N,ExeEdge>::OutIterator {
  public:
    inline Successor(const N* node)
      : GenGraph<N,ExeEdge>::OutIterator(node) {}
  };
...
/*     trace = true; */
  ...
  for (InstIterator inst(sequence) ; inst ; inst++)  {
	...
	const elm::genstruct::Table<otawa::hard::Register *>& reads = node->inst()->inst()->readRegs();
