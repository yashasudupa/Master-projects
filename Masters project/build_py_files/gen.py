		...
		try:
			...
			for mod in self.get_tobuild():
				...
				self.env.writeRule(
					...
					"%s-all" % mod.name,
					" ".join(
							[self.map[m].name + "-install" for m in (mod.requirements + mod.uses) if self.map.__contains__(m) and self.map[m] in self.get_toinstall()] + 
							[self.map[m].name + "-all" for m in (mod.requirements + mod.uses) if self.map.__contains__(m) and self.map[m] in self.get_tobuild() and self.map[m] not in self.get_toinstall()] + 
							[".setup-%s" % mod.name]),
